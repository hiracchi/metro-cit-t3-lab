...
3. APIのみでOllamaを利用する場合と、Webインターフェースを導入して利用する場合では、運用面や利用者体験にどのような違いがあるか。
4. Webフロントエンド、バックエンド、AI APIの3要素は、それぞれどのような役割を担い、どのようにデータを受け渡しているか説明せよ。
5. このシステムを学外に広く公開する場合、どのようなセキュリティリスクが考えられるか。
6. 構築したネットワークサーバーを運用し続ける場合、OSやソフトウェアのアップデート（`update`コマンド等）を定期的に行う必要がある。不具合を放置してアップデートを怠った場合に生じる問題点や法的な責任について考察せよ。
7. `docker run` で個別に起動する方法と、`docker compose` で複数サービスを管理する方法では、設定変更や再現性の面でどのような違いがあるか説明せよ。
