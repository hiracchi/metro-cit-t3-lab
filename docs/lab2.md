...
## 4. 課題
1. ファイアウォール設定にて、ポート22（SSH）と80（HTTP）を許可した理由を説明せよ。また、不要なポートを開放したままにすることの危険性を考察せよ。
2. パスワード認証を禁止し、鍵認証のみに制限するセキュリティ上の理由は何か。
3. SSHポート（22/tcp）を開放した状態で安全に運用するために、どのような設定や運用ルールが必要か説明せよ。
4. `scp` によるファイル転送を用いる利点と、転送前後に確認すべき項目（転送先、権限、上書き有無など）を説明せよ。
